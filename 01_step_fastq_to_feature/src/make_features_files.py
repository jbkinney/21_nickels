...
while not stop:
    ...
    #           if (not write_result1 and write_result2):
    #               read1_not_parsed = read1_not_parsed + 1
    #           if (not write_result2 and write_result1):
    #               read2_not_parsed = read2_not_parsed + 1
    #           if (not write_result2 and not write_result1):
    #               both_read_not_parsed = both_read_not_parsed + 1
    ...
    for key in features_list:
        ...
        # if match found between UMI and BARCODE, save length of the variable
        # region in the output dictionary under key var_length.
        # if 'var' in out_dict.keys():
        #    out_dict['var_length'] = len(out_dict['var'])
        # else:
        #    out_dict['var_length'] = 0
...
# append var length key to features_list list
#features_list.append('var_length')
...
repp = pd.DataFrame({'num_reads': [num_reads],
                     'n_tag': [n_tag],
                     'read1_not_parsed': [read1_not_parsed],
                     'read2_not_parsed': [read2_not_parsed],
                     'both_read_not_parsed': [both_read_not_parsed],
                     'sample_read': [len(df_res)]})


features_list = list(np.sort(features_list).copy())
features_list.insert(0,'sample')
df_res = df_res[features_list].copy()
#df_res = df_res[['sample', 'var', 'umi', 'bc', 'var_length']].copy()


df_res.to_csv(out_file, sep='\t', index=False, na_rep='None')

repp.to_csv(out_file + '.report', sep='\t', index=False)
